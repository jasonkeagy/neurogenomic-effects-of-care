...
file <- "dataInput_ParentalCare_2019-08-27.RData"
load(file)

# load results from step 2.a.2
file <- "stickle-networkConstr-auto_dataInput_ParentalCare_2019-08-27_8_30_0.15_3_0.995"
load(file)
...
GreenX <- ggplot(summaryTable[summaryTable$module == "MEgreen", ], 
  ...
  coord_flip() +
  ggtitle("green module eigengene expression") + 
  ylim(c(ggplot_build(module)$layout$panel_scales_y[[1]]$range$range)) + 
  geom_errorbar(aes(ymin = mean - SE, ymax = mean + SE, colour = CareTrt), width = 0, linewidth = 2, position = position_dodge(width = 1)) + 
  theme_bw() +
  scale_color_discrete(type = colors, name = "Care\nTreatment", labels = c("Orphaned", "Father-reared")) + 
  scale_shape_discrete(labels = c("Female", "Male")) + 
  ylab("mean +/- SE Eigengene Expression") + 
